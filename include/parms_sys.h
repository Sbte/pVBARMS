...
typedef enum SOLVERTYPE {SOLFGMRES, SOLGMRES} SOLVERTYPE;
typedef enum PARAMTYPE  {MAXITS, KSIZE, DTOL, NEIG} PARAMTYPE;
typedef enum MATTYPE    {MAT_NULL=-1, MAT_VCSR=0, MAT_CSR=1, MAT_B_VCSR=2} MATTYPE;//updated 
typedef enum PCTYPE     {PCBJ, PCSCHUR, PCRAS} PCTYPE;
typedef enum PCILUTYPE  {PCILU0, PCILUK, PCILUT, PCARMS, PCVBARMS, PCVBILUT, PCVBILUK, PCVBARMSOLD} PCILUTYPE;//updated
...
typedef struct parms_FactParam {
  ...
  /*
    ...
    lfil     = lfil[0:6] is an array containing the fill-in parameters.
      ...
      ...
      execpt lfil[7] = level of vbiluk in vbarms.
      etc..
