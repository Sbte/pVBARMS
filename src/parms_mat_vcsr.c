...
static int MatVec_vcsr(parms_Mat self, FLOAT *x, FLOAT *y)
{
  ...
  //outputcsmatpa(matrix,"csmatinmatvec.coo",1);//int outputvbmat( vbsptr vbmat, char *filename, int onebase)
...
static int MatGetDiag_vcsr(parms_Mat self, void **mat)//why dont use *csptr intead of void
{
