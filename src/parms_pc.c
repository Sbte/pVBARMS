                ...
                parms_Operator *op)
{
    ...
    else if(type == PCVBILUT)
        ierr = parms_vbilut_vcsr(self->A, param, mat, op);
    else if(type == PCVBILUK)
        ierr = parms_vbiluk_vcsr(self->A, param, mat, op);
    else
    {
    ...
    if (ierr) {
        printf("ERROR: Can't construct a preconditioner\n");
        PARMS_ABORT(16);
    }
    return 0;
}
...
int parms_PCILUGetName(parms_PC self, char **iluname)
{
    if(self->pcilutype == PCILU0)
        ...
        *iluname = "ARMS";
    else if(self->pcilutype == PCVBARMS)//NEW
        *iluname = "VBARMS";
    else if(self->pcilutype == PCVBARMSOLD)//NEW
        *iluname = "VBARMSOLD";
    else if(self->pcilutype == PCVBILUT)//NEW
        *iluname = "VBILUT";
    else if(self->pcilutype == PCVBILUK)//NEW
        *iluname = "VBILUK";
    else
 ...
 * @return 0 on success.
 */
int parms_PCCreate_b(parms_PC *self, parms_Mat A)
{
    parms_PC new_pc;
    int i;
 ...
 * @return 0 on success.
 */
int parms_PCSetType_b(parms_PC self, PCTYPE pctype)
{
    parms_Map    is;
    parms_Mat    A;
    BOOL         isserial;
    ...
    if (isserial) {
        ...
        if(pctype == PCBJ)
            ...
            parms_PCCreate_b_Schur(self);
        else if(pctype == PCRAS)
            parms_PCCreate_b_RAS(self);
        else
        {
 ...
 * @return 0 on success.
 */
int parms_PCSetup_b(parms_PC self)
{
    ...
    if(self->istypeset == false){
        parms_PCSetType_b(self, PCBJ);//block version, setup the block operations for bj preconditioner
    }
    ...
    else
    {
        ...
        self->ops->pc_free(&self);
        parms_PCSetType_b(self, self->pctype);
        self->issetup = true;
        self->ops->setup(self);
    }
    return 0;
 ...
 * @return 0 on success.
 */
int parms_PCFree_b(parms_PC *self)
{
    (*self)->ref--;
    if ((*self)->ref == 0 ) {
        (*self)->ops->pc_free(self);
