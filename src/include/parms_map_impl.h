...
struct parms_Map_ {
  ...
  int         lsize;		/**< number of local variables, in block version, it turns into the block dimension of the local matrix*/
  ...
  */     
  ...
  parms_Table vstable;	
//-----------------------------------------------------------dividing line--------------------------------------------------------
  int *nB;//new, specially for block version preconditioner, to store the diagonal block size.??change to gbsz could be better??work on this later
  int llsize;//new, specially for block version preconditioner, the point dimension of the local matrix.
  BOOL isBversion;//vbsptr	
  int *pperm;//local point permutation array with size llsize
  int *lbsz_before;//local block index array before local permutation
  int *lbsz_after;//local block index array after local permutation
};
