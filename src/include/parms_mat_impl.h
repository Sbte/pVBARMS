...
typedef struct VBSpaFmt {
    ...
    BData **ba;   /* pointer-to-pointer to store nonzero blocks, A(i,j)=column by column instead of row by row */
    BData *D;     /* to store inversion of diagonals            */
    int lsize;    /* specially for offd in parallel code        */
} VBSparMat, *vbsptr;

typedef vbsptr parms_bvcsr;

//typedef parms_dvcsr dcsptr;//new
//---------------------------------------------------------------------------dividing line-------------------------------------------------------------------------------------- 
...
extern int parms_MatFree_b_dvcsr(parms_Mat *self);
extern int parms_MatFree_b_vcsr(parms_Mat *self);
