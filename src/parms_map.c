...
int parms_MapFree(parms_Map *self)
{
    ...
    if ((*self)->ref == 0 ) {
        ...
        if ((*self)->nB) {
            PARMS_FREE((*self)->nB);//new
        }
        if ((*self)->lbsz_before) {
            PARMS_FREE((*self)->lbsz_before);//new
        }
                           ...
                           vtype)
{
    ...
    newMap->n_ext = 0;
    newMap->nB    =  NULL;//new
    newMap->pperm    =  NULL;//new
    newMap->lbsz_before = NULL;//new
    newMap->lbsz_after = NULL;//new
...
//-------------------------------------------------------------------------dividing line--------------------------------------------
/** 
 * assign the dimension of each block to the nB member of map struct
 *
 */
int parms_Map_Assign_blockstructure(parms_Map self, int *nB)
{
    self->nB = nB;
    return 0;
}
/** 
 * Get the size of point variables on the local PE.
 *
 * Return the size of point variables not vertices on the local PE.
 *
 * @param self A parms_Map object.
 *
 * @return The size of variables on the local PE.
 */
int parms_MapGetLocallSize(parms_Map self)
{
    return self->llsize;
}
