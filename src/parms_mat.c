		       ...
		       int *ja, FLOAT *values, INSERTMODE mode)  
{
  ...
  is       = self->is;
  offset    = is->start;//c or fortran start index
  ...
  if (isserial) {
	...
	if (found) {
	  if (mode == INSERT) {
	    ra[index] = values[j];//and the value assigning of local nB here.
	  }
	  else if (mode == ADD) {
	    ra[index] += values[j];
	  }
    ...
    perm    = is->perm;
//output_intvectorpa("mapperm",perm,0, lsize);
//printf("numinf = %d, offset = %d\n",is->ninf,offset);getchar();
    numinf  = is->ninf;
...
//printf("rindex = %d, rp = %d\n",rindex,*rp);//getchar();
	...
	for (j = start; j < end; j++) {
	  ...
	  if (cp != NULL && *cp < lsize) {
//printf("rindex = %d, rp = %d, cindex = %d, cp = %d \n",rindex,*rp, cindex, *cp);//getchar();
	    /* local column index */
	    lcindex = perm[*cp];
