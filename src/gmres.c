...
/*--------------------------------------------------------------------
  gmres_create    : create the gmres solver. 
  gmres_free      : free the memory for the gmres solver. 
  gmres_setksize  : set the restart size of the gmres solver.
  gmres_view      : dump the gmres solver.
  parms_gmres     : the gmres solve function.

  $Id: gmres.c,v 1.4 2006-12-15 07:02:07 zzli Exp $
  ------------------------------------------------------------------*/
...
typedef struct gmres_data {
  int restart;
  int neigs;
} *gmres_data;

#define TINY 1.0e-20
int parms_gmres(parms_Solver self, FLOAT *y, FLOAT *x)
{
    BOOL outflag, intflag;
    int i, i1, pti, pti1, ptih, j, restart, its;
    int ii, jj, k, k1, maxits, nloc, size, one = 1;
    FLOAT  *vv, *z, *hh, *s, *rs, *rvec, alpha, t1;
 ...
 * s and rs and hh above --------------------------------*/    
    ...
    outflag = true;
    while(outflag) {
/*------- Left preconditioning: compute Mr = b-Ax ------ */
/*------ compute Ax = rvec -------*/
      parms_MatVec(A, x, rvec);

/*-------- compute residual vector: rvec = b-Ax --------*/
      parms_VecAYPX(rvec, y, -1.0, is);      

/*--------- Precondition residual vector ---------------*/
      parms_PCApply(pc, rvec, vv);
/*--------- compute the norm of the residual -----------*/
      parms_VecGetNorm2(vv, &ro, is);
//      if(fabs(ro-DBL_EPSILON) <= DBL_EPSILON*fabs(ro)) {
...
/*---------- compute A*vv[i] = z --------------*/
       parms_MatVec(A, &vv[pti], z);
       
/*---------- apply preconditioner -------------*/
       parms_PCApply(pc, z, &vv[pti1]);
 ...
 * get solution */
     ...
     for (j = 0; j <= i; j++) {
	t1 = rs[j];
	parms_VecAXPY(x, &vv[j*nloc], t1, is);
      }
      /* test for return */
...
static int gmres_getresidual(parms_Solver self, FLOAT *y, FLOAT *x, FLOAT *res)
{
...
static int gmres_getresidualnorm2(parms_Solver self, FLOAT *y, FLOAT *x, REAL *rnorm)
{
...
static int gmres_free(parms_Solver *self)
{
  gmres_data fdata;
...
static int gmres_view(parms_Solver self, parms_Viewer v)
{
  FILE *fp;
  char *name, *iluname;
  int restart;
...
static int gmres_setksize(parms_Solver self, int restart)
{
  gmres_data fdata;
...
static int gmres_setneig(parms_Solver self, int neigs)
{
  gmres_data fdata;
...
static struct parms_Solver_ops parms_gmres_sol = {
  parms_gmres,
  gmres_getresidual,
  gmres_getresidualnorm2,
  gmres_setksize,
  gmres_setneig,
  gmres_free,
  gmres_view
 ...
 *  \return 0 on success.
 */
int gmres_create(parms_Solver self)
{
  gmres_data fdata;
  ...
  fdata->neigs = 0;
  self->data =fdata;
  PARMS_MEMCPY(self->ops, &parms_gmres_sol, 1);		
