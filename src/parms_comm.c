...
int parms_CommCreate(parms_Comm *self, MPI_Comm comm)
{
  ...
  /*members specially for block version preconditioner*/
  comm_handler->vlist_send_b    = NULL;
  //int          *vlist_send_b;	//!< list of variables to be sent, point version in the block case.
  comm_handler->ptrvsend_b    = NULL;
  comm_handler->ptrvrecv_b    = NULL;
  comm_handler->buf_send_b    = NULL;
  comm_handler->buf_recv_b    = NULL;
...
int parms_CommFree(parms_Comm *self)
{
  ...
  if ((*self)->ref == 0 ) {
    if ((*self)->npsend) {
      ...
      if ((*self)->vlist_send_b) 
        PARMS_FREE((*self)->vlist_send_b);//new
      //printf("(*self)->buf_send = %p\n",(*self)->buf_send);
      if ((*self)->buf_send) 
        PARMS_FREE((*self)->buf_send);
      if ((*self)->buf_send_b)
        PARMS_FREE((*self)->buf_send_b);//new   
      if ((*self)->isdt_alloc) {
    ...
    if ((*self)->ptrvsend_b)
      PARMS_FREE((*self)->ptrvsend_b);//new
    PARMS_FREE((*self)->ptrvrecv);
    if ((*self)->ptrvrecv_b)
      PARMS_FREE((*self)->ptrvrecv_b);//new
    if ((*self)->nprecv) {
      ...
      if ((*self)->buf_recv_b)
        PARMS_FREE((*self)->buf_recv_b);//new
    }
...
int parms_CommView(parms_Comm self, parms_Viewer v)
{
  ...
  for (i = 0; i < self->nprecv; i++) {
    fprintf(fp, "Receiving data from processor%d, self->mdata_send = %d, self->nodv = %d, self->ptrvrecv[i+1] = %d\n", self->procs_recv[i], self->mdata_send, self->nodv,self->ptrvrecv[i+1]);
  }

  for (i = 0; i < self->nodv; i++) {
    fprintf(fp, "i=%d, self->odvlist[i]=%d\n",i, self->odvlist[i]);
  } 
  fprintf(fp, "========================================\n");
 ...
 * @return 0 on success.
 */
int parms_CommGetRecvBuf_b(parms_Comm self, FLOAT **rbuf)
{

  *rbuf = self->buf_recv_b;
  return 0;
}
...
//------------------------------------------------------------------------------------------dividing line---------------------------------------------------
/** 
 * Start communication among neighbouring processors(block version).
 *
 * If the self->ctype is set to P2P, then the data are copied to the
 * send buffer, self->buf_send, otherwise, the data are sent directly.
 *
 ...
 * @return 0 on success.
 */
int parms_CommDataBegin_b(parms_Comm self, void *data, int pos)
{
    FLOAT *data_send;
    MPI_Comm comm;
    int i, j, index, start, end, length, tag;
...
#if defined(DBL_CMPLX)  
    for (i = 0; i < self->nprecv; i++) {
        start = self->ptrvrecv_b[i];
        end = self->ptrvrecv_b[i+1];
        length = end - start;
        MPI_Irecv(&self->buf_recv_b[start], length, MPI_CMPLX,
                  self->procs_recv[i], tag, comm, &self->req_recv[i]);
    }
    //if (self->ctype == P2P) {
    for (i = 0; i < self->npsend; i++) {
        start  = self->ptrvsend_b[i];
        end    = self->ptrvsend_b[i+1];
        length = end - start;
        /* copy data to the send buffer */
        for (j = start; j < end; j++) {
            index = self->vlist_send_b[j];
            self->buf_send_b[j] = data_send[index-pos];
        }
        MPI_Isend(&self->buf_send_b[start], length, MPI_CMPLX,
                  self->procs_send[i], tag, comm, &self->req_send[i]);
    }
    //}
    /*else if (self->ctype == DERIVED) {
    ...
    for (i = 0; i < self->nprecv; i++) {
        ...
        length = end - start;
        MPI_Irecv(&self->buf_recv_b[start], length, MPI_DOUBLE,
                  self->procs_recv[i], tag, comm, &self->req_recv[i]);//int MPI_Irecv(void *buf, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, MPI_Request *request)
    }
    //if (self->ctype == P2P) {
    for (i = 0; i < self->npsend; i++) {
        ...
        MPI_Isend(&self->buf_send_b[start], length, MPI_DOUBLE,
                  self->procs_send[i], tag, comm, &self->req_send[i]); //int MPI_Isend(void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *request)
    }
    //}
    /*else if (self->ctype == DERIVED) {
