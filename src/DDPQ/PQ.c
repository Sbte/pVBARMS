...
int PQperm(csptr mat, int *Pord, int *Qord, int *nnod, 
           double tol, int nbnd)
{
    /*---------------------------------------------------------------------
...
|---------------------------------------------------------------------*/ 
    ...
    i = 1;
    preSel(mat, icor, jcor, i, tol, &count, nbnd) ;
...
{
    /*----------------------------------------------------------------------
|   adds element nod schur complement
|---------------------------------------------------------------------*/
    iord[nod] = *nback;
    riord[*nback] = nod;
    (*nback)--;
...
//    printf("in indset.\n");//%f %p %s %c
    ...
    for(j=0; j<n; j++)
        iord[j] = -1;
    for(j = n-1; j >=nbnd; j--) {//for 1:n spilted into two loops, in interface nodes loop, the nod doest ++
    ...
    for (nod=0; nod<n; nod++) {
        while (iord[nod] != -1)   {
            if (++nod >= mat->n) goto label50;//last node also does not have to do anything
        }
...
|---------------------------------------------------------------------*/
        ...
        while (jcount < bsize && prog) {
            ...
            last0 = last;
            jcount0 = jcount;
//            printf("begin value is %d, last value is %d.\n", begin, last0);//%f %p %s %c
...
//                printf("jnod value is %d.\n", jnod);//%f %p %s %c
                ...
                gmat = mat;
                for (k=0; k<2; k++) {//0 and 1 for unsymm
                    rowj = gmat->pj[jnod];
                    for (j=0; j<gmat->nnzrow[jnod]; j++) {
                        jcol = rowj[j];
                        if (iord[jcol] == -1 ) {//marker
            ...
            lastlev = begin;
            begin = last0+1;
//            printf("jcount=%d, prog = %d, jcount0=%d, in the loop\n",jcount, prog, jcount0); getchar();

        }
//        printf("bsize = %d, jcount=%d\n",bsize, jcount); getchar();
        /*-----------------------------------------------------------------------
...
|--------------------------------------------------------------------*/
    ...
    for (irow=0; irow<n; irow++) {
        ...
        tdia = 0.0;
        for (k=0; k<kz; k++) {
            if (kj[k] == irow) tdia = ABS_VALUE(kr[k]);
            tnorm += ABS_VALUE(kr[k]);
        }
        if (tnorm > 0.0)
            tnorm =  tdia / tnorm;
        w[irow] = tnorm;
...
|---- end of weightsC -------------------------------------------------
|--------------------------------------------------------------------*/
int preSel(csptr mat, int *icor, int *jcor, int job, double tol, int
           *count, int nbnd)
{
    /*---------------------------------------------------------------------
...
|--------------------------------------------------------------------*/
    int i, k, kmax, col, jmax, countL;
    int *nz, *nzi, *jcol;
    FLOAT *mrow, t1;
    double rownorm, *weight, t, tmax, wmax;

    /*--------------------begin */
    /*-----------------------------------------------------------------------*/
    nz =mat->nnzrow;
    weight = (double *) malloc(nbnd*sizeof(double));
    nzi    = (int *)malloc(nbnd*sizeof(int));
    ...
    for (i=0; i<nbnd; i++) {
        ...
        for (k = 0; k<nz[i]; k++) {
            ...
            if (col < nbnd) {
                nzi[i]++;
                t = ABS_VALUE(mrow[k]);
                if(ABS_VALUE(t-DBL_EPSILON) > DBL_EPSILON*ABS_VALUE(t)) {
                    rownorm += t;
                    if (tmax < t) {
                        tmax = t;
                        kmax = k;
