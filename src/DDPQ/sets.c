...
void *Calloc( int n, int size, const char *msg )
{
    ...
    if (ptr == NULL)
        errexit( "Not enough mem for %s. Requested size: %d bytes", msg, n * size );
...
int lev4_nnz(p4ptr levmat, int *lev) 
{
    ...
    if (*lev == 0)
        printf("\nnnz/lev used:      L        U        F        E    subtot\n");
    printf("    Level %2d %8d %8d %8d %8d %8d\n",
           *lev, nnzL, nnzU, nnzF, nnzE, nnzT);
    (*lev)++;
    nextmat = levmat->next;
    if (nextmat != NULL)
 ...
 *---------------------------------------------------------------------*/
    ...
    setupVBMat( vbmat, nBlk, nB );//setupVBMat1( vbmat, nBlk, nBlk, nB, nB );george
    ...
    for( i = 0; i < n; i += nB[b_row] ) {
        ...
        for( j = 0, ipos = 0; j < nBlk; j++ ) {
            if( iw[j] != 0 ) {
                //printf("\n brow=%d,ipos=%d,j=%d",b_row,ipos,j); getchar();
        ...
        for( j = i; j < i+nB[b_row]; j++ ) {
            for( k = 0; k < csmat->nnzrow[j]; k++ ) {
                ...
                //		printf(" ipos=%d \n",ipos);// getchar();
                br = j - i;
                bc = csmat->pj[j][k] - vbmat->bsz[b_col];
                //printf("\n br=%d,bc=%d,b_row=%d,csmatv=%f",br,bc,b_row,csmat->ma[j][k]); getchar();
                DATA(vbmat->ba[b_row][ipos],nB[b_row],br,bc) = csmat->pa[j][k];
            }
        }
NEXT_ROW:
        /* reset iw */
...
int csrvbsrC_new( int job, int nBlk, int *nB, csptr csmat, vbsptr vbmat )
{
    /*----------------------------------------------------------------------
   *  Compressed C-style Sparse Row to C-style Various Block Sparse Row
   *----------------------------------------------------------------------
   ...
   *---------------------------------------------------------------------*/
    int n, i, j, k;
    int nnz, szofBlock, ipos, b_row, b_col, br, bc, counter;
    int *iw = NULL, *p2bmap = NULL, *bcindex = NULL, *bsz;
    ...
    bsz = vbmat->bsz;
    iw = (int *)Malloc( sizeof(int)*nBlk, "csrvbsrC_1" );
    p2bmap = malloc(n*sizeof(*p2bmap));//mapping between point and block index
    bcindex = malloc(nBlk*sizeof(*bcindex));//auxillary array to store one row's column index


    for (i = 0; i < nBlk; i++){
        for (j = bsz[i]; j < bsz[i+1]; j++ )
            p2bmap[j] = i;
        //printf("i = %d \n",i);
        //memset(&p2bmap[bsz[i]], i, (bsz[i+1]-bsz[i])*sizeof(int));
        //printf("bsz[i] = %d, bsz[i+1]= %d, p2bmap[bsz[i]] = %d \n",bsz[i], bsz[i+1], p2bmap[bsz[i]]); getchar();
    }
    /* output_intvector("p2bmap.coo",p2bmap,0, n); */
    /* exit(1); */
    ...
    for( i = 0; i < n; i += nB[b_row] ) {
        ...
        for( j = i; j < i+nB[b_row]; j++ ) {
           ...
           of the original matrix */
                //b_col = col2vbcol( csmat->pj[j][k], vbmat );
                b_col = p2bmap[csmat->pj[j][k]];//get the block index by mapping
                if( iw[b_col] == 0 ) {
                    //iw[b_col] = 1;
                    counter = vbmat->nzcount[b_row]++;
                    bcindex[counter] = b_col;
                    iw[b_col] = counter+1;
                }
            }
        }
        if( 0 == ( nnz = vbmat->nzcount[b_row] ) ) continue;
        vbmat->ja[b_row] = (int *)Malloc( sizeof(int)*nnz, "csrvbsrC_new_2" );
        memcpy(vbmat->ja[b_row], bcindex, nnz*sizeof(int));
        /* calculate the pattern of the (b_row)-th row of the block matrix */
        /* for( j = 0, ipos = 0; j < nBlk; j++ ) { */
        /*   if( iw[j] != 0 ) { */
        /*     //printf("\n brow=%d,ipos=%d,j=%d",b_row,ipos,j); getchar(); */
...
int nnzVBMat( vbsptr vbmat )//the total number of nozero blocks in the matrix
{
...
int nnzVBMat1( vbsptr vbmat )//the number of nonzero point entries in the block form matrix 
{
    int mem = 0, nnz, i, j, n = vbmat->n, *bsz = vbmat->bsz;
    int *ja, dimR, dimC, col, k;
    BData *ba;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            col = ja[j];
            dimC = B_DIM(bsz, col-vbmat->lsize);
            for (k = 0; k < dimC * dimR; k++)
                if (ba[j][k]) mem++;
        }
    }
    //printf("nnz original matrix=%d\n", mem);
    return mem;
}

int memVBMat( vbsptr vbmat )//the total amount of point entries in the all nozero blocks
{
    ...
    for( i = 0; i < n; i++ ) {
        ...
        dm = 0;
        for( j = 0; j < nnz; j++ ) {
            dm += B_DIM(bsz,vbmat->ja[i][j]-vbmat->lsize);//lsize is specially for offd matrix
        }
        mem += dm * B_DIM(bsz,i);
    }
    return mem;
...
int memVBMat1( vbsptr vbmat )//the total amount of individual entries in the all nozero blocks, for rectangular block, george
{
    int mem = 0, nnz, i, j, n = vbmat->n, *bsz = vbmat->bsz,*bszc = vbmat->bszc, dm;
    for( i = 0; i < n; i++ ) {
        nnz = vbmat->nzcount[i];
        dm = 0;
        for( j = 0; j < nnz; j++ ) {
            dm += B_DIM(bszc,vbmat->ja[i][j]-vbmat->lsize);
        }
        mem += dm * B_DIM(bsz,i);
    }
    return mem;
...
|--------------------------------------------------------------------*/
    ...
    for( j = 0; j < nzcount; j++ ) {
        ncol = lu->L->ja[nrow][j];
        szOfBlock = B_DIM(bsz,nrow)*B_DIM(bsz,ncol)*sizeof(FLOAT);
        lu->L->ba[nrow][j] = (BData)Malloc( szOfBlock, "mallocVBRow" );
    }

    szOfBlock = sizeof(FLOAT) * B_DIM(bsz,nrow) * B_DIM(bsz,nrow);
...
void zrmC( int m, int n, BData data ){//set the block to zero
    int mn = m * n, i;
...
{
    /*----------------------------------------------------------------------
| Initialize VBSpaFmt structs, for exculsively rectangular matrix.george
|----------------------------------------------------------------------
| on entry:
|==========
| ( vbmat ) =  Pointer to a VBSpaFmt struct.
|       n   =  size of block matrix
|      nBR   = row size of diagonal block, so the real row size of the matrix
|              is nBR[0] + nBR[1] + ... + nBR[n-1]
|              do nothing if nBR is NULL
|      nBC   = column size of diagonal block, so the real column size of the matrix
|              is nBC[0] + nBC[1] + ... + nBC[n-1]
|              do nothing if nBC is NULL
|      
|
| On return:
|===========
...
|--------------------------------------------------------------------*/
    ...
    //printf("\n vbmat->n=%d,vbmat->nc=%d",vbmat->n,vbmat->nc); getchar();
    //printf("\n nBBR[0]=%d,nBBR[1]=%d,nBBR[2]=%d,nBBR[3]=%d,nBBR[4]=%d,nBBR[5]=%d",nBR[0],nBR[1],nBR[2],nBR[3],nBR[4],nBR[5]); getchar();
    //output_intvector("nBBC.coo" ,nBC,0,nc );
    //output_intvector("nBBR.coo" ,nBR,0,1 );

    if( nBR ) {
        vbmat->bsz = (int *)Malloc( sizeof(int)*(n+1), "setupVBMat" );
        vbmat->bsz[0] = 0;
        for( i = 1; i <= n; i++ ) {
            vbmat->bsz[i] = vbmat->bsz[i-1] + nBR[i-1];
            //printf("\n vbn=%d",vbmat->bsz[i]);
        }
    } else
        vbmat->bsz = NULL;
    ...
    //printf("\n nc=%d,i=%d",nc,i); getchar();

    //printf("\n nc=%d",nc); getchar();
    if( nBC ) {
        //printf("\n nc=%d",nc); getchar();
        vbmat->bszc = (int *)Malloc( sizeof(int)*(nc+1), "setupVBMat" );
        vbmat->bszc[0] = 0;
        //printf("\n vbn=%d,i=%d",vbmat->bsz[i],i); getchar();
        for( i = 1; i <= nc; i++ ) {
            vbmat->bszc[i] = vbmat->bszc[i-1] + nBC[i-1];
            //printf("\n vbmat->bszc[i]=%d",vbmat->bszc[i]);
        }
    } else
        vbmat->bszc = NULL;
    //printf("\n vbn=%d",vbmat->bsz[i]); getchar();
    vbmat->nzcount = (int *)Malloc( sizeof(int)*n, "setupVBMat" );
    vbmat->ja = (int **)Malloc( sizeof(int *)*n, "setupVBMat" );
    vbmat->ba = (BData **)Malloc( sizeof(BData *) * n, "setupVBMat" );
    vbmat->D = NULL;
    //printf("\n vbn=%d,vbnc=%d",vbmat->n,vbmat->nc); getchar();
    return 0;
