...
//                   lu->D[jrow], &nn, &beta1, lu->bf, &mm );
            GGEMM( "n", "n", mm, nn, nn, alpha1, L->ba[i][j], mm,
                   lu->D[jrow], nn, beta1, lu->bf, mm );
...
//                           L->ba[i][jpos], &mm );
                    GGEMM( "n", "n", mm, kk, nn, alpha2, L->ba[i][j],
                           mm, U->ba[jrow][k], nn, beta2,
                           L->ba[i][jpos], mm );
                } else if( col == i ) {
...
//                           &mm, U->ba[jrow][k], &nn, &beta2,
//                           lu->D[i], &mm );
                    GGEMM( "n", "n", mm, mm, nn, alpha2, L->ba[i][j],
                           mm, U->ba[jrow][k], nn, beta2,
                           lu->D[i], mm );
                } else {
                    kk = B_DIM(bsz,col);
...
//                           U->ba[i][jpos], &mm );
    ...
    lu->DiagOpt = 2;
    //printf("\n lu->DiagOpt=%d",lu->DiagOpt); getchar();
    return 0;
}
