...
c-----------------------------------------------------------------------
c     does a trivial mapping of a square grid into a virtual mpx x mpy 
c     processor grid. One way overlapping allowed:
c     when assigning a block as a subdomain then `ovlp'
c     extra lines to the right and `ovlp' extra lines to the 
c     top of the subrectangle being consired are added.
c-----------------------------------------------------------------------
c on entry:
c--------- 
...
c            (number of lines) in each direction. One=way overlap
...
c-----------------------------------------------------------------------
     ...
     &     valfmt*20, rhsfmt*20, rhstyp*3, guesol*2
      integer totcrd, ptrcrd, indcrd, valcrd, rhscrd, nrow, ncol,
     &     nnz, neltvl, nrhs, nmax, nzmax, nrwindx, stlen
      integer ia (nmax+1), ja (nzmax)
      real*8 a(nzmax), rhs(*)
      character fname*100
c-----------------------------------------------------------------------
...
c assign the value to guesol here for job=0 case
        guesol = rhstyp(2:3)
      else
c if no RHS initialize the output variables to zero.
        nrhs = 0
        guesol = "  "
      end if
...
c
      len = nrhs*nrow
c
      if (len*nvec .gt. lenrhs*nrow) then
        ierr = 5
        goto 12
      endif
c
...
c
      if (guesol(1:1) .eq. 'G' .or. guesol(1:1) .eq. 'g') then
        next = next+len
        iend = iend+ len
c       read(iounit,valfmt) (rhs(i), i = next, iend)
        read(iounit,rhsfmt) (rhs(i), i = next, iend)
      endif
c
c     read exact solutions if available
...
c-----------------------------------------------------------------------
c     some routines extracted/ modified from SPARSKIT2
c
c     The original subroutines have been modified to support 
c     the complex case.
c-----------------------------------------------------------------------
      subroutine zwreadmtc (nmax,nzmax,job,fname,length,a,ja,ia,rhs,nrhs,
     *     guesol,nrow,ncol,nnz,title,key,type,ierr)
c-----------------------------------------------------------------------
c     this  subroutine reads  a boeing/harwell matrix,  given the
c     corresponding file. handles right hand sides in full format
...
c-----------------------------------------------------------------------
      ...
      fname1 = ""
      fname1(1:length) = fname(1:length) 
      call zreadmtc(nmax,nzmax,job,fname1,a,ja,ia,rhs,nrhs,
     *     guesol,nrow,ncol,nnz,title,key,type,ierr)  
